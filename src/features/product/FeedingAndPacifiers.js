...
import Button from '@mui/material/Button';
  ...
  const FeedingAndPacifiers = () => {
    ...
    const fetchData = async () => {
      try {
        let res = await amountProduct("FeedingAndPacifiers");
        const amountOfPages =
          res.data.count % 12 === 0
            ? res.data.count / 12
            : Math.floor(res.data.count / 12) + 1;
        setPageCount(amountOfPages); // קביעת מספר העמודים הכולל
        getProductByCategory("FeedingAndPacifiers", page, 12).then((res) => {
          setArr(res.data);
        });
      } catch (err) {
    ...
    return (
      <>
        <img src="../../pictures/הנקהוהאכלהערכתנושא.png"></img>
        <h1>האכלה ומוצצים</h1>
  ...
  export default FeedingAndPacifiers;