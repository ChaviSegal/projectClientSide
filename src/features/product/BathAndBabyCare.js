...
const BathAndBabyCare = () => {
  ...
  const fetchData = async () => {
    try {
      let res = await amountProduct("BathAndBabyCare");
      const amountOfPages =
        res.data.count % 12 === 0
          ? res.data.count / 12
          : Math.floor(res.data.count / 12) + 1;
      setPageCount(amountOfPages); // קביעת מספר העמודים הכולל
      getProductByCategory("BathAndBabyCare", page, 12).then((res) => {
        setArr(res.data);
      });
    } catch (err) {
  ...
  return (
    <>
      <img src="../../pictures/אמבטוטיפולבתינוקערכתנושא.png"></img>
      <h1>אמבט וטיפול בתינוק</h1>
...
export default BathAndBabyCare;
